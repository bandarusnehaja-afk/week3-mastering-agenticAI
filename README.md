...
| `tools.get_executive_summary` | **Yes — LLM decides to call it, and the tool itself makes a second LLM call to write the prose** | chat agent |
...
| Streamlit **Chat** tab | **Yes — same `create_react_agent` as `main.py chat`, same 7 tools** | `streamlit_app.py::render_chat` |
...
### `tools.get_executive_summary()` (LLM-invoked, and it calls the LLM again internally)

No arguments. The one tool that isn't a pure data read: `service.get_executive_summary`
pulls the latest snapshot, stuck-task list, and week-over-week trends from SQLite — the same
inputs `render_report`'s `--narrative` flag uses — computes the same stats dict, and calls
`report_template.generate_narrative` to turn it into 3-4 sentences of prose. That function is
shared code: both this tool and `--narrative` send the identical prompt (see
[Prompt Library](#3-narrative-summary-prompt-pm_agentreport_templatepygenerate_narrative)),
so the two paths can't drift into saying different things about the same data. Verified live
with both exact phrasing ("give me an executive summary") and indirect phrasing ("how are we
doing overall? TL;DR") — the LLM picked this tool correctly both times over the six other
data-only tools. Because this is the one tool with a second LLM call inside it, its error handling is
narrower than the other five: a missing API key raises `RuntimeError` from
`config.get_llm()`, which the tool wrapper catches and returns as a plain message ("No
report has been generated yet...", or the key error text) instead of crashing the turn. Any
other LLM failure (rate limit, network) is *not* caught here and propagates as an ordinary
exception through the agent's tool-calling loop — different from `_llm_narrative`'s
blanket `except Exception` fallback text in the deterministic report path.
...
fragment the model sees when deciding what to call — these seven strings are effectively the
...
| `get_executive_summary` | "Generate a short (3-4 sentence) LLM-written executive summary of the latest weekly report's overall health, covering total tasks, high-risk count, tasks stuck 2+ sprints, and week-over-week completions/new blockers. Use this when asked for 'an executive summary', 'a TL;DR of the sprint', or 'how are we doing overall'. For a task-by-task breakdown use get_current_sprint_snapshot instead — this tool only produces prose." |
...
### 3. Narrative summary prompt (`pm_agent/report_template.py::generate_narrative`)

One shared prompt template behind two call sites: `report --narrative` (via `_llm_narrative`)
and the chat tool `get_executive_summary` (via `service.get_executive_summary`). Both compute
the same stats dict from their respective data source and pass it through the identical
completion call, so the deterministic report and the chat agent can never describe the same
sprint's health differently:
